...
PDF tagged structure links content streams to the structure tree via Marked Content IDs (MCIDs). Key concepts for accessibility auditing:
...
Reading order is determined by the structure tree's depth-first traversal order, not by the visual position of content on the page. This distinction is critical for accessibility:

- **Structure tree order** — the intended reading sequence as declared by the authoring tool. What screen readers follow.
- **Content stream order** — the order text operators appear in the page's content stream. Usually matches visual left-to-right, top-to-bottom, but not always (especially for multi-column layouts, sidebars, and callout boxes).
...
PDF/UA governs universal accessibility. Key requirements:
...
PDF/A is an ISO standard (19005) for long-term archival. Relevant to accessibility checking because PDF/A level 'a' requires tagged structure:
...
Fonts without `/ToUnicode` CMaps can't be reliably extracted to text by screen readers or search indexers. Auditing approach:
...
**Strengths:** Excellent low-level access to PDF objects via `context.enumerateIndirectObjects()`. Can directly read `PDFRawStream`, `PDFDict`, `PDFArray`, etc.

**Limitations:**
- No built-in content stream parser.
- `PDFNumber` value access is inconsistent — use `Number(val.toString())` for reliable extraction.
- **Cannot read metadata streams.** pdf-lib only reads the `/Info` dictionary for metadata. PDFs that store metadata exclusively in XMP metadata streams (common in modern tools) return `undefined` from `getTitle()`, `getAuthor()`, etc. Read XMP directly from raw stream bytes.
...
Used for stream decompression when parsing content streams. Key functions:
- `inflateSync(data)` — decompression of raw DEFLATE only (RFC 1951). Fast path for most streams.
- `decompressSync(data)` — decompression with auto-format detection (GZIP, Zlib, or raw DEFLATE). Fallback when `inflateSync` fails on pako-produced zlib streams.

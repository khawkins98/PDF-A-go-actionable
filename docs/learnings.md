...
**Key gotcha — `let`/`const` TDZ in helper functions:** If a helper function (e.g., `addNewPage()`) resets a `let` variable (e.g., `nextMcid = 0`), that variable must be **declared before the first call** to the helper. Function declarations are hoisted but `let`/`const` bindings are not — calling the function before the `let` line executes triggers a Temporal Dead Zone ReferenceError. This survives dev mode too, but is easy to miss if the code path isn't exercised during testing.
